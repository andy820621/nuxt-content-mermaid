...
::callout{variant="warning" title="displayMode is not a fence attribute in v3.0.1"}
The legacy `{displayMode="compact"}` shorthand is no longer supported. Configure compact Gantt layout through Mermaid YAML frontmatter instead:
...
Mermaid's official top-level YAML `displayMode` metadata also remains valid. This module leaves both official YAML forms in the diagram source for Mermaid to interpret.
::
...
The YAML `title`, `displayMode`, and `config` values remain Mermaid-owned, while this module converts `toolbar` into a built-in renderer prop. This YAML is frontmatter inside the Mermaid fence, not the Content frontmatter at the top of the page. For Mermaid's complete YAML-frontmatter schema, use the [Mermaid configuration documentation](https://mermaid.js.org/config/configuration.html).
