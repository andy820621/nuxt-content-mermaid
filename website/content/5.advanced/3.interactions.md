...
`copy` copies Mermaid source. The control briefly announces `Copied` or `Copy failed`; the renderer first uses the Clipboard API and then attempts the browser's legacy copy fallback. `fullscreen` is displayed only when the browser supports the Fullscreen API. `expand` opens the in-page overlay.
...
`margin` keeps pixels around the expanded SVG. Set `expand: false` to disable the feature entirely. See [Configuration](/configuration#configure-expand-behavior) for the boolean-preset and object-patch distinction.

| Control | Default | Effect |
| --- | --- | --- |
| `diagramClick` | `true` | Clicking a rendered diagram opens the overlay. |
| `esc` | `true` | Escape closes the overlay. |
| `wheel` | `true` | A non-modified wheel action on the overlay background closes it. |
| `swipe` | `true` | A one-finger swipe closes the overlay. |
| `overlayClick` | `true` | Clicking the overlay background closes it. |
| `closeButtonClick` | `true` | Shows the button labelled “Minimize diagram” and lets it close the overlay. |
