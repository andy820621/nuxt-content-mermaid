...
The package initializes Mermaid with `startOnLoad: false`, the `default` theme, `Arial, sans-serif, 微軟正黑體`, and `securityLevel: 'strict'`. Values in `loader.init` override that baseline. See [Writing Diagrams](/writing-diagrams#configuration-precedence) for page and diagram overrides.
