...
在 Node 應用程式中，Nuxt Content 可能需要資料庫連接器。`better-sqlite3`、`sqlite3` 與受支援 Node.js 版本內建的 SQLite 都是常見選擇，但選擇權屬於您的應用程式與 Nuxt Content，不屬於本模組。請依已安裝的版本參考 [Nuxt Content 安裝指南](https://content.nuxt.com/docs/getting-started/installation)，確認其支援的連接器。
