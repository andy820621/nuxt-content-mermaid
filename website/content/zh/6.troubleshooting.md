...
pnpm v10 以上版本可能會封鎖原生連接器，直到您執行 `pnpm approve-builds` 或將所選連接器加入 `pnpm.onlyBuiltDependencies`。請先排除連接器失敗，再檢查本模組。安裝選擇詳見[開始使用](/zh/getting-started#安裝)。
...
若這可運作，請逐步還原原始碼，並用 Mermaid 文件驗證語法。若無法運作，請檢查瀏覽器主控台與建置輸出，然後確認頁面的 Content 集合會保留 `config` 欄位，如[撰寫圖表](/zh/writing-diagrams#頁面與圖表設定)所述。

無效的圍欄中繼資料會刻意保留為原始碼。請確認行內屬性只使用 `title`、`config` 與 `toolbar`，且 Mermaid YAML 前置資料是含有成對 `---` 分隔符號的有效 YAML。
