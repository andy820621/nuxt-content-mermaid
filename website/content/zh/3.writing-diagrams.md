...
::callout{variant="warning" title="v3.0.1 不再支援 displayMode fence 屬性"}
舊有的 `{displayMode="compact"}` shorthand 已不再支援。請改在 Mermaid YAML frontmatter 中設定 Gantt compact layout：
...
Mermaid 官方的頂層 YAML `displayMode` 中繼資料也仍然有效；本模組會將這兩種官方 YAML 寫法原樣留給 Mermaid 解讀。
::
...
YAML 中的 `title`、`displayMode` 與 `config` 仍由 Mermaid 擁有；`toolbar` 則由本模組轉成內建渲染器的 prop。
...
Nuxt Content 會依 collection schema 決定哪些 frontmatter 欄位會成為頁面的頂層資料。
若未宣告 `config`，它不會成為頁面的頂層欄位，因此本模組無法取得 `pageConfig`，頁面上的 Mermaid 圖表也不會套用這組設定。請務必依下方範例在 schema 中宣告此欄位。
::
...
Page Mermaid Config 會穿越 Content 傳輸邊界（transport），因此只能包含嚴格的純資料。
若應用程式程式碼需要僅限用戶端的 Mermaid 能力（capability），請使用[直接 Mermaid 設定](#mermaid-元件)。
...
`loader.init`、Page Mermaid Config、Direct Mermaid Config、Mermaid YAML 前置資料與指令都可攜帶 Mermaid 設定。本模組會保留 Mermaid 擁有的資料，但不會重新定義其選項結構描述、主題名稱、安全性設定或指令語意。
詳細內容請參閱 [Mermaid 設定文件](https://mermaid.js.org/config/configuration.html)。
