...
內建渲染器接受預設插槽作為原始碼備援內容、`#loading` 作為初始載入狀態，以及 `#error="{ error, source }"` 作為 Mermaid 渲染失敗。或者可由 `components.error` 提供全域內建錯誤元件，並接收 `error` 與 `source`；自訂渲染器不會使用它。

## 註冊外部排列引擎

公開的 `$mermaid()` injection 可以註冊上游 Mermaid 擴充，不需要取代內建渲染器。例如，需要 ELK 的應用程式可以安裝 [issue #88](https://github.com/andy820621/nuxt-content-mermaid/issues/88) 評估過的版本組合：

```bash
pnpm add mermaid@11.16.1 @mermaid-js/layout-elk@0.2.2
```

接著從僅在用戶端執行的 application plugin 註冊上游 layout definitions：
...
這是進階的上游整合方式，不是本套件擁有的 ELK 功能或相容性契約。application plugin 呼叫 `$mermaid()` 會在應用程式啟動時載入 Mermaid；大型 ELK renderer 仍會等 Mermaid 實際選到它時才載入。升級時請自行維持 Mermaid 與 layout package 的版本相容。若 plugin 缺席或註冊失敗，上游 Mermaid 可能只顯示警告並改用 Dagre，而不會產生應用程式錯誤。
